...
namespace clang {
  ...
  /// Extract which transformation to apply from a TransformExecutableDirective and its clauses.
  ...
  struct ExtractTransform {
   ...
   const TransformExecutableDirective* Directive;
    ...
    ExtractTransform(  ASTContext& ASTCtx, const TransformExecutableDirective* Directive) : ASTCtx(ASTCtx), Directive(Directive) { assert(Directive); }
    ...
    ClauseTy* assumeSingleClause() {
      ClauseTy* Result = nullptr;
      for (ClauseTy* C : Directive->getClausesOfKind<ClauseTy>()) {
        if (Result) {
          DiagError(C->getBeginLoc(), diag::err_sema_transform_clause_one_max) <<   TransformClause::getClauseKeyword( C->getKind()) << C->getRange();
          break;
          }
   ...
   llvm::Optional<int64_t> evalIntArg(Expr *E, int MinVal) {
      ...
      if (!Res.Val.isInt()) {
          DiagError(E->getExprLoc(), diag::err_sema_transform_clause_arg_expect_int);
          return None;
      }
      llvm::APSInt Val = Res.Val.getInt();
      int64_t Int = Val.getSExtValue();
      if (Int < MinVal) {
        DiagError(E->getExprLoc(), diag::err_sema_transform_clause_arg_min_val) << MinVal << SourceRange(E->getBeginLoc(), E->getEndLoc());
        return None;
      }
      return Int;
    }
   ...
   void allowedClauses(ArrayRef<TransformClause::Kind> ClauseKinds) {
     ...
     for (TransformClause* C : Directive->clauses()) {
       assert(llvm::find(ClauseKinds, C->getKind()) != ClauseKinds.end() && "Parser must have rejected unknown clause");
     }
#endif
   }
  ...
  case Transform::LoopUnrollingKind: {
    ...
    if (Full && Partial)
      DiagError(Full->getBeginLoc(), diag::err_sema_transform_unroll_full_or_partial);
    ...
    if (Full) {
      ...
      if (AnyError || !Factor.hasValue())
        return nullptr;
      return wrap( LoopUnrollingTransform::createPartial(Loc, false, true, true,  Factor.getValue()));
    }
  ...
  case Transform::LoopUnrollAndJamKind: {
    ...
    if (Partial) {
      ...
      if (AnyError || !Factor.hasValue())
        return nullptr;
      return wrap( LoopUnrollAndJamTransform::createPartial(Loc, false, true,   Factor.getValue()));
    }
  ...
  case clang::Transform::LoopInterleavingKind: {
    ...
    return wrap( LoopInterleavingTransform::Create(Loc, true, InterleaveFactor));
  }
  default:
