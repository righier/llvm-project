...
protected:
  TransformedTree(llvm::ArrayRef<Derived *> SubLoops, Derived *BasedOn,
      ...
      : Subloops(SubLoops.begin(), SubLoops.end()), Original(Original),
        BasedOn(BasedOn), FollowupRole(FollowupRole), Stage(Stage) {}
...
private:
  ...
  public:
    ...
    VisitTransformExecutableDirective(const TransformExecutableDirective *S) {
      // Can happen in semantic analysis in non-instantiated templates.
      if (!S->getTransform())
        return false;

      const Stmt *TheLoop = getAssociatedLoop(S->getAssociated());
      Transforms.emplace_back(S->getTransform(),
  ...
  struct TransformApplicator {
    ...
    void checkStageOrder(ArrayRef<NodeTy *> PrevLoops, Transform *NewTrans) {
      ...
      for (auto PrevLoop : PrevLoops) {
        auto PrevStage = PrevLoop->getStage();
    ...
    NodeTy *applyUnrolling(LoopUnrollingTransform *Trans, NodeTy *MainLoop) {
      ...
      int Stage = Trans->getLoopPipelineStage();
      ...
      if (Trans->isFull()) {
        ...
        NodeTy *All =
            Builder.createFollowup(MainLoop->Subloops, MainLoop,
                                   LoopUnrollingTransform::FollowupAll, Stage);
        NodeTy *Unrolled = Builder.createFollowup(
            MainLoop->Subloops, MainLoop,
            LoopUnrollingTransform::FollowupUnrolled, Stage);
        NodeTy *Remainder = Builder.createFollowup(
            MainLoop->Subloops, MainLoop,
            LoopUnrollingTransform::FollowupRemainder, Stage);
    ...
    NodeTy *applyUnrollAndJam(LoopUnrollAndJamTransform *Trans,
      ...
      if (!Inner) {
        ...
        NodeTy *UnrolledOuter = Builder.createFollowup(
            {}, MainLoop, LoopUnrollAndJamTransform::FollowupOuter, Stage);
      ...
      NodeTy *UnrolledOuter = Builder.createFollowup(
          {Inner}, MainLoop, LoopUnrollAndJamTransform::FollowupOuter, Stage);
    ...
    NodeTy *applyDistribution(LoopDistributionTransform *Trans,
      ...
      NodeTy *All =
          Builder.createFollowup(MainLoop->Subloops, MainLoop,
                                 LoopDistributionTransform::FollowupAll, Stage);
    ...
    applyVectorizeInterleave(LoopVectorizationInterleavingTransform *Trans,
      ...
      NodeTy *All = Builder.createFollowup(
          MainLoop->Subloops, MainLoop,
          LoopVectorizationInterleavingTransform::FollowupAll, Stage);
      NodeTy *Vectorized = Builder.createFollowup(
          MainLoop->Subloops, MainLoop,
          LoopVectorizationInterleavingTransform::FollowupVectorized, Stage);
      NodeTy *Epilogue = Builder.createFollowup(
          MainLoop->Subloops, MainLoop,
          LoopVectorizationInterleavingTransform::FollowupEpilogue, Stage);
    ...
    NodeTy *applyVectorize(LoopVectorizationTransform *Trans,
      ...
      auto *VecInterleaveTrans = LoopVectorizationInterleavingTransform::create(
          Trans->getLoc(), false, false, true, false, Trans->getWidth(),
    ...
    NodeTy *applyInterleave(LoopInterleavingTransform *Trans,
                            NodeTy *MainLoop) {
      auto *VecInterleaveTrans = LoopVectorizationInterleavingTransform::create(
          Trans->getLoc(), false, false, false, true, 1, None,
...
protected:
  TransformedTreeBuilder(ASTContext &ASTCtx,
                         llvm::SmallVectorImpl<NodeTy *> &AllNodes)
      : ASTCtx(ASTCtx), AllNodes(AllNodes) {}

  NodeTy *createRoot(llvm::ArrayRef<NodeTy *> SubLoops) {
    auto *Result = new NodeTy(SubLoops, nullptr, nullptr, -1, -1);
  ...
  NodeTy *createPhysical(llvm::ArrayRef<NodeTy *> SubLoops,
                         clang::Stmt *Original) {
    assert(Original);
    auto *Result = new NodeTy(SubLoops, nullptr, Original, -1, 0);
    AllNodes.push_back(Result);
    return Result;
  }

  NodeTy *createFollowup(llvm::ArrayRef<NodeTy *> SubLoops, NodeTy *BasedOn,
                         int FollowupRole, int Stage) {
    assert(BasedOn);
    auto *Result = new NodeTy(SubLoops, BasedOn, nullptr, FollowupRole, Stage);
    AllNodes.push_back(Result);
    return Result;
  }
