...
// RUN: %clang_cc1 -triple x86_64-pc-windows-msvc19.0.24215 -std=c99 -emit-llvm -O3 -mllvm -polly -mllvm -polly-position=early -mllvm -polly-process-unprofitable -mllvm -polly-reschedule=0 -mllvm -polly-postopts=0 -mllvm -polly-pragma-ignore-depcheck -debug-info-kind=limited -gno-column-info -Rpass=polly-opt-isl %s  -o /dev/null 2>&1 >/dev/null | FileCheck %s --check-prefix=WARN
