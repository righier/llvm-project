...
// RUN: %clang_cc1 -triple x86_64-pc-windows-msvc19.0.24215 -std=c99 -emit-llvm -O3 -mllvm -polly -mllvm -polly-position=early -mllvm -polly-process-unprofitable -mllvm -polly-reschedule=0 -mllvm -polly-postopts=0 -debug-info-kind=limited     %s -o /dev/null 2>&1 >/dev/null | FileCheck %s --check-prefix=FAILURE
...
// RUN: %clang_cc1 -triple x86_64-pc-windows-msvc19.0.24215 -std=c99 -emit-llvm -O3 -mllvm -polly -mllvm -polly-position=early -mllvm -polly-process-unprofitable -mllvm -polly-reschedule=0 -mllvm -polly-postopts=0 -mllvm -polly-pragma-ignore-depcheck -debug-info-kind=limited -Rpass=polly-opt-isl %s  -o /dev/null 2>&1 >/dev/null | FileCheck %s --check-prefix=WARN
...
// FAILURE: pragma-id-interchange-illegal.c:9:1: warning: not applying loop interchange: cannot ensure semantic equivalence due to possible dependency violations [-Wpass-failed=polly-opt-isl]
...
// WARN: pragma-id-interchange-illegal.c:9:1: remark: Could not verify dependencies for loop interchange; still applying because of -polly-pragma-ignore-depcheck [-Rpass=polly-opt-isl]
