...
// from GCIHelper.h. The shortened from_*() to not work well with overload resolution.
//
//===----------------------------------------------------------------------===//
...
namespace polly {
  ...
  static inline auto intersect_range(isl::multi_union_pw_aff MUPwAff, isl::set Set) {
    return isl::manage(isl_multi_union_pw_aff_intersect_range(MUPwAff.release(), Set.release()));
  }

  static inline auto flat_domain_product(isl::union_map UMap1, isl::union_map UMap2) {
    return isl::manage(isl_union_map_flat_domain_product(UMap1.release(), UMap2.release()));
  }

  static inline auto flat_range_product(isl::union_map UMap1, isl::union_map UMap2) {
    return isl::manage(isl_union_map_flat_range_product(UMap1.release(), UMap2.release()));
  }

  static inline auto flat_range_product(isl::basic_map BMap1, isl::basic_map BMap2) {
    return isl::manage(isl_basic_map_flat_range_product(BMap1.release(), BMap2.release()));
  }
  ...
  static inline isl::size dim(const isl::local_space& LS, isl::dim Dim) {
    return isl::manage(isl_local_space_dim(LS.get(), static_cast<enum isl_dim_type> (Dim)));
  }

  static inline auto get_coefficient_val(const isl::constraint& C, isl::dim Dim, int Pos) {
    return isl::manage(isl_constraint_get_coefficient_val(C.get(), static_cast<enum isl_dim_type> (Dim), Pos));
  }
  ...
  static inline auto set_coefficient_val(isl::constraint C, isl::dim Dim, int Pos, isl::val V) {
    return isl::manage(isl_constraint_set_coefficient_val(C.release(), static_cast<enum isl_dim_type> (Dim), Pos, V.release()));
  }
  ...
  namespace detail {
    ...
    public:
      ...
      static isl::union_map convert(isl::multi_union_pw_aff MUPwAff) {
        return isl::manage(isl_union_map_from_multi_union_pw_aff(MUPwAff.release() )  );
      }
    };

    template<>
    class Converter <isl::map> {
    public:
